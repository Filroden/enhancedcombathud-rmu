...
RMUData.setOpenSkillsCategory = (catOrNull) => {
    ...
    if (catOrNull) SKILLS_OPEN_CAT.set(id, String(catOrNull));
    else SKILLS_OPEN_CAT.delete(id);
};
...
RMUData.setOpenSpellState = (typeOrNull, nameOrNull) => {
    const id = getActiveTokenId();
    if (typeOrNull) SPELLS_OPEN_LIST_TYPE.set(id, typeOrNull);
    else SPELLS_OPEN_LIST_TYPE.delete(id);
    if (nameOrNull) SPELLS_OPEN_LIST_NAME.set(id, nameOrNull);
    else SPELLS_OPEN_LIST_NAME.delete(id);
};
...
RMUData.ensureExtendedTokenData = async function () {
    ...
    if (doc && typeof doc.hudDeriveExtendedData === "function") {
        try {
            ...
            console.warn("[ECH-RMU] hudDeriveExtendedData failed:", e);
        }
    }
};
...
RMUData.bucketOf = function (att) {
    ...
    if (sName.includes("unarmed") || sName.includes("strikes") || sSpec.includes("unarmed") || sSpec.includes("strikes") || type.includes("natural") || att?.isNatural === true) return "natural";
...
RMUData.getGroupedSkillsForHUD_All = function () {
    ...
    const all = RMUData.getAllActorSkills(actor).map(RMUData.toDisplaySkill).filter(Boolean);
            ...
            for (const spell of spellList.spells) {
                ...
                if (spell.known === true && spell.spellAttack && spell.spellAttack.chart?.name) {
                    ...
                    const attack = {
                        ...
                        _spellListInfo: `${spellList.spellListName} (${listTypeKey})`,
                        _isSpellAttack: true,
