...
export function definePortraitPanel(CoreHUD) {
    ...
    class RMUPortraitPanel extends Base {
        ...
        /**
         * Renders a confirmation dialogue and resets the movement pace penalty tracking.
         * @param {Event} event - The click event.
         * @private
         */
        async _onResetMovementDialog(event) {
            event.preventDefault();
            event.stopPropagation();
            ...
            const confirmed = await DialogV2.confirm({
                window: {
                    title: "Reset Pace Penalty",
                    icon: "fa-solid fa-person-running",
                },
                content:
                    "<p>Are you sure you want to reset your pace penalty tracker? This will set your current 'high tide' movement mark to zero for the purpose of calculating penalties for new actions while moving.</p>",
                classes: ["enhancedcombathud-rmu"],
                rejectClose: false,
                modal: true,
            });
            ...
            if (!targetToken) {
                ui.notifications.warn("[ECH-RMU] No valid token found on the canvas to reset movement.");
                return;
            }

            const doc = targetToken.document;

            // Calculate current total round movement to set as the new "start" point
            ...
            // Reset the high water mark AND the current phase start distance
            await doc.update({
                "flags.enhancedcombathud-rmu.maxCompletedPhases": 0,
                "flags.enhancedcombathud-rmu.phaseStartDist": currentTotal,
            });

            ui.notifications.info("Pace penalty tracker reset.");
        }

        /**
         * Injects the custom "Set Defenses" and "Reset Pace" buttons into the Argon player panel.
        ...
        async activateListeners(element) {
            ...
            const setDefensesIconPath = foundry.utils.getRoute("modules/enhancedcombathud-rmu/icons/guardian.svg");
            ...
            resetMoveButton.classList.add("player-button");
            resetMoveButton.dataset.tooltip = "Reset Pace Penalty";

            const resetPaceIconPath = foundry.utils.getRoute("modules/enhancedcombathud-rmu/icons/sprint.svg");
...
export function defineMovementHud(CoreHUD) {
    ...
    class RMUMovementHud extends Base {
        ...
        get _actorData() {
            return getActorMovementData(this.actor, this.token?.document);
        }
        ...
        async updateMovement() {
            ...
            if (!this.element.querySelector(".rmu-tactical-info")) {
                ...
                trackContainer.className = "rmu-track-container";
                ...
                if (existingSpaces && !existingSpaces.classList.contains("phase-track")) {
                    existingSpaces.replaceWith(trackContainer);
                } else if (!this.element.querySelector(".phase-track")) {
